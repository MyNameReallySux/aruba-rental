...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(350), __webpack_require__(350)))
...
var Scroller = exports.Scroller = function Scroller() {
    var _this = this;
    ...
    (0, _jquery2.default)(window).on('scroll resize load', function (e) {
        _this.handleScrollEvent(e);
    });
};

Scroller.guid = 0;
Scroller.registry = {};
Scroller.lastScrollPos = 0;
Scroller.defaultOptions = {
    ...
    onInitElement: function onInitElement() {},
...
Scroller._addToRegistry = function ($element, config) {
    var guid = Scroller._getNewGuid();
    Scroller.registry[guid] = _extends({
...
Scroller._getNewGuid = function () {
    var guid = Scroller.guid;
    Scroller.guid++;
    return guid;
};

Scroller._getScrollDetails = function () {
    var pos = (0, _jquery2.default)(window).scrollTop();
    var direction = pos > Scroller.lastScrollPos ? 'down' : 'up';
    var delta = Math.abs(Scroller.lastScrollPos - pos);
    var lastPos = Scroller.lastScrollPos;

    return { pos: pos, direction: direction, delta: delta, lastPos: lastPos };
};

Scroller._getElementDetails = function ($element) {
...
Scroller._getElementVisibleDetails = function ($element, elementPos, scrollPos, offset) {
    var top = elementPos.top,
        bottom = elementPos.bottom,
        height = elementPos.height;
    var pos = scrollPos.pos,
        direction = scrollPos.direction,
        delta = scrollPos.delta;


    var viewportHeight = (0, _jquery2.default)(window).height();

    var partialFromTop = pos <= top + height - offset;
    var partialFromBottom = pos + viewportHeight >= top + offset;

    var fullFromTop = pos <= top - offset;
    var fullFromBottom = pos + viewportHeight >= top + height + offset;
...
var _initialiseProps = function _initialiseProps() {
    this.initializeConfig = function (options) {
        options.map(function (config) {
            config = Object.assign({}, Scroller.defaultOptions, config);

            var _config = config,
                selector = _config.selector,
                offset = _config.offset,
                topOffset = _config.topOffset,
                bottomOffset = _config.bottomOffset,
                onInitElement = _config.onInitElement,
                onInit = _config.onInit;


            try {
                if (!selector) throw new TypeError('Scroller selector \'' + selector + '\' was either blank or invalid, must be a {String}');
                var $selection = (0, _jquery2.default)(selector);

                if ($selection.length <= 0) (0, _consoleUtils.warn)('Scroller selector \'' + selector + '\' found no reference element.');
                if ($selection.length > 1) (0, _consoleUtils.warn)('Scroller selector \'' + selector + '\' found more than 1 reference element, using first element.');
                ...
                (0, _consoleUtils.error)(err.message);
            }
        });
    };

    this.handleScrollEvent = function (e) {
        var scrollPos = Scroller._getScrollDetails();
        Object.values(Scroller.registry).map(function (entry) {
            var $element = entry.$element,
                selector = entry.selector,
                triggered = entry.triggered,
                onScroll = entry.onScroll,
                onScrollUp = entry.onScrollUp,
                onScrollDown = entry.onScrollDown,
                onTopIn = entry.onTopIn,
                onTopOut = entry.onTopOut,
                onBottomIn = entry.onBottomIn,
                onBottomOut = entry.onBottomOut,
                offset = entry.offset,
                topOffset = entry.topOffset,
                bottomOffset = entry.bottomOffset;


            var elementPos = Scroller._getElementDetails($element);
            var elementVis = Scroller._getElementVisibleDetails($element, elementPos, scrollPos, offset);

            var callbackArgs = { elementPos: elementPos, scrollPos: scrollPos, offset: offset };
            var partial = elementVis.partial,
                full = elementVis.full,
                viewport = elementVis.viewport;
            ...
            if (scrollPos.direction == 'up') {
                ...
                var shouldFireOnTopIn = scrollPos.pos >= elementPos.bottom - offset;
                var notTriggeredOnTopIn = !triggered.topIn;
                if (shouldFireOnTopIn && notTriggeredOnTopIn) {
                    onTopIn($element, callbackArgs);
                    triggered.topIn = true;
                }

                var shouldFireOnBottomOut = scrollPos.pos + viewport.height >= elementPos.bottom - offset;
                var notTriggeredOnBottomOut = !triggered.bottomOut;
            ...
            if (scrollPos.direction == 'down') {
                ...
                var shouldFireOnTopOut = scrollPos.pos >= elementPos.top - offset;
                var notTriggeredOnTopOut = !triggered.topOut;
                if (shouldFireOnTopOut && notTriggeredOnTopOut) {
                    onTopOut($element, callbackArgs);
                    triggered.topOut = true;
                }

                var shouldFireOnBottomIn = scrollPos.pos + viewport.height >= elementPos.bottom - offset;
                var notTriggeredOnBottomIn = !triggered.bottomIn;
